...
def lambda_handler(event, context):
    ...
    repo_registery_data = get_s3_object(s3_client, settings.bucket, repo_registry_path)
    repo_registery_data = upsert_repo_registry(run_datetime, data, repo_registery_data)
    save_json_to_s3(s3_client, repo_registery_data, settings.bucket, repo_registry_path)
